...
| D12 — resolved addon-capable experimental identity | The additive addon surface is assigned `CarbonLuau 0.4.0-experimental`; package, API, ABI, provider protocol, schema and Luau identities remain separate. | Requalify affected public behavior and assign an explicit migration/version decision for breaks |
...
| D15 - resolved GUI Foundation 1 retained presentation model; runtime closure implemented through 1F | The retained GUI authority, ownership, presentation, interaction, publication, reconciliation, recovery and scope rules are specified below. [GuiFoundation1.md](GuiFoundation1.md) owns supporting rationale and implementation guidance; [GuiFoundation1A.md](GuiFoundation1A.md), [GuiFoundation1B.md](GuiFoundation1B.md), [GuiFoundation1C.md](GuiFoundation1C.md), [GuiFoundation1D.md](GuiFoundation1D.md), [GuiFoundation1E.md](GuiFoundation1E.md) and [GuiFoundation1F.md](GuiFoundation1F.md) record the internal substrate, retained runtime, static projection, bounded automatic synchronization, secure Activated ingress and lifecycle/runtime closure without claiming a GUI release identity. | Complete authenticated-client qualification and explicit release planning before assigning a release/API identity or advertising support |
...
The gameplay facade introduced as `CarbonLuau 0.3.0-experimental` remains compatible. Foundation E assigns the additive addon-capable scripting identity `CarbonLuau 0.4.0-experimental`. It includes package-qualified `require("@id")` and `require("@id/path")`, existing local `require("path")`, `addon.Id`/`addon.Version`, `addon:IsDependencyAvailable(id)` for a declared binding, and inherently public `main`.
...
The scripting API and package identity for GUI Foundation 1 are **UNASSIGNED / release-planning gated**. D12's existing `0.4.0-experimental` addon identity remains unchanged and does not include GUI. No package, scripting API, native ABI, provider protocol or package-schema bump is implied by this architecture adoption.
