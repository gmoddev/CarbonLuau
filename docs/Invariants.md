...
| D15 — resolved GUI Foundation 1 retained presentation model; unimplemented | The retained GUI authority, ownership, presentation, interaction, publication, reconciliation, recovery and scope rules are specified below. [GuiFoundation1.md](GuiFoundation1.md) owns supporting rationale and implementation guidance, not competing policy. | Implement only through an explicitly scoped GUI phase; qualify the complete boundary before assigning a release/API identity or advertising support |
