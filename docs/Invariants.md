...
| D4 — Phase 1 resolved; later mechanics deferred | One global VM per generation and fresh sandboxed environment per loaded chunk. No persistence between chunks or module cache yet. Callback environment reuse, module sharing, cycles/depth and cache keys remain future decisions. | Before module/callback features |
...
| D7 — resolved Phase 1 transaction | Candidate executes the restricted deterministic smoke before swap. Its log output is staged and discarded on failure. Successful swap destroys the old generation once. No gameplay side effects exist; future effects need a new transaction contract. | Before later host capabilities |
| D8 — native ABI resolved; scripting API deferred | Native ABI 1.0 encodes major/minor in uint32; managed validates major before runtime binding. Package 0.1.0 and exact Luau revision are separate identities. Public scripting API compatibility/deprecation remains deferred. | Before first public scripting API |

**Phase 1 implementation:** the gates above are resolved only for the scoped execution core. [Phase1-Validation.md](Phase1-Validation.md) owns actual qualification results and limitations. D5/D6 and later module/callback/gameplay mechanics remain deferred; Phase 1 does not establish their safety.
