    ...
    public partial class CarbonLuau
    {
        ...
        private void RunPhase3Fixture()
        {
            ...
            try {
                ...
                string Script = "local P=game:GetService('Players'); local C=game:GetService('Commands'); local Old=P:GetPlayers()[1]; P.PlayerAdded:Connect(function(V) assert(V.UserId=='"+UserId+"' and V.IsConnected); V:SendMessage('welcome fixture'); print('joined') end); P.PlayerRemoving:Connect(function(V) assert(not V.IsConnected); assert(not pcall(function() V:SendMessage('stale') end)); print('removed') end); C:Register('hello',{permission='carbonluau.example.hello'},function(Ctx) assert(Ctx.Player.UserId=='"+UserId+"'); Ctx.Player:SendMessage('hello fixture'); print('A '..(Ctx.Arguments[1] or 'empty')) end)";
                Load(Script);
                ...
                Load("local P=game:GetService('Players'); local Old=P:GetPlayers()[1]; assert(Old.IsConnected); assert(not pcall(function() Old:SendMessage('provisional') end)); P.PlayerRemoving:Connect(function(V) assert(not Old.IsConnected and not V.IsConnected); print('old invalid') end); P.PlayerAdded:Connect(function(V) assert(not Old.IsConnected and V.IsConnected and Old~=V); print('new lifetime') end)");
